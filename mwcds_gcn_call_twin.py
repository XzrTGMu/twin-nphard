...
class DPGAgent(Solver):
    ...
    def foo_train(self, adj_0, wts_0, train=False):
        ...
        with tf.GradientTape() as g:
            ...
            act_val, act = self.act(state, train, explore=0.002)
    ...
    def predict_train(self, adj, zs_0, state, n_samples=1, z_std=0.15):
        ...
        with tf.GradientTape() as g:
            ...
            for i in range(n_samples):
                wts = 10.0*np.random.uniform(0.01, 1.01, size=(nn, 1))
                ...
                mwcds_i, mwds_i, total_wt_i = heuristic_func(adj_0, gcn_wts)
                solu = list(mwcds_i)
                ind_vec[solu, 0] += wts[solu, 0]/(10.1*float(n_samples))
