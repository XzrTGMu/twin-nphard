...
for j in range(test_len):
    graph, adj_0, wts_0 = random_graph(size=sizes[j], k=ks[j], p=ps[j], gtype=FLAGS.gtype, gseed=j)
    test_instances.append((graph, adj_0, wts_0, sizes[j], ks[j], ps[j]))
...
for epoch in range(FLAGS.epochs):
    ...
    for id in np.random.permutation(2000):
        ...
        for j in range(test_len):
            ...
            top_wts = np.clip(gcn_wts, a_min=0.0, a_max=None)
            mwcds_0, mwds_0, total_wt_0 = heuristic_func(adj_1, wts_1)
            mwcds_i, mwds_i, _ = heuristic_func(adj_1, top_wts)
            total_wt_i = np.sum(wts_1[list(mwcds_i)])
